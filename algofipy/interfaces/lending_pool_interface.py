...
class LendingPoolInterface:
    ...
    def get_burn_quote(self, amount):
        ...
        asset1_burned_amount = self.market1.b_asset_to_asset_amount(b_asset1_burned_amount).underlying
    ...
    def get_burn_txns(self, user, quote, params=None):
        ...
        # send lp asset
        txn0 = get_payment_txn(user.address, params, self.address, -1 * quote.lp_delta, self.lp_market.underlying_asset_id)
        ...
        txn2 = ApplicationNoOpTxn(
            ...
            index=self.app_id,
            app_args=[
                bytes(LENDING_POOL_INTERFACE_STRINGS.burn_step_2, "utf-8")
            ],
            accounts=[self.pool.address],
            foreign_apps=[self.pool.application_id, self.pool.manager_application_id],
            foreign_assets=[self.pool.asset1.asset_id, self.pool.asset2.asset_id, self.pool.lp_asset_id]
        ...
        txn3 = ApplicationNoOpTxn(
            ...
            index=self.app_id,
            app_args=[
                bytes(LENDING_POOL_INTERFACE_STRINGS.burn_step_3, "utf-8")
            ],
            accounts=[user.address, self.market1.address],
            foreign_apps=[self.market1.app_id, self.market1.manager_app_id],
            foreign_assets=[self.market1.underlying_asset_id, self.market1.b_asset_id]
        ...
        txn4 = ApplicationNoOpTxn(
            ...
            index=self.app_id,
            app_args=[
                bytes(LENDING_POOL_INTERFACE_STRINGS.burn_step_4, "utf-8")
            ],
            accounts=[user.address, self.market2.address],
            foreign_apps=[self.market2.app_id, self.market2.manager_app_id],
            foreign_assets=[self.market2.underlying_asset_id, self.market2.b_asset_id]
    ...
    def get_swap_txns(self, user, quote, max_slippage, is_swap_for_exact=False, params=None):
        ...
        txn2 = ApplicationNoOpTxn(
            ...
            app_args=[
                ...
                int_to_bytes(self.market1.underlying_asset_id) if input_is_asset1 else int_to_bytes(self.market2.underlying_asset_id)
            ],
            accounts=[user.address, self.market1.address] if input_is_asset1 else [user.address, self.market2.address],
            foreign_apps=[self.market1.app_id, self.market1.manager_app_id] if input_is_asset1 else [self.market2.app_id, self.market2.manager_app_id],
            foreign_assets=[self.market1.underlying_asset_id, self.market1.b_asset_id] if input_is_asset1 else [self.market2.underlying_asset_id, self.market2.b_asset_id]
        )
        ...
        txn3 = ApplicationNoOpTxn(
            ...
            app_args=[
                ...
                int_to_bytes(self.market1.b_asset_id) if input_is_asset1 else int_to_bytes(self.market2.b_asset_id),
                bytes(LENDING_POOL_INTERFACE_STRINGS.swap_for_exact, "utf-8") if is_swap_for_exact else bytes(LENDING_POOL_INTERFACE_STRINGS.swap_exact_for, "utf-8"),
        ...
        txn4 = ApplicationNoOpTxn(
            ...
            index=self.app_id,
            app_args=[
                bytes(LENDING_POOL_INTERFACE_STRINGS.swap_step_4, "utf-8")
            ],
            accounts=[user.address, self.market1.address],
            foreign_apps=[self.market1.app_id, self.market1.manager_app_id],
            foreign_assets=[self.market1.underlying_asset_id, self.market1.b_asset_id]
        ...
        txn5 = ApplicationNoOpTxn(
            ...
            index=self.app_id,
            app_args=[
                bytes(LENDING_POOL_INTERFACE_STRINGS.swap_step_5, "utf-8")
            ],
            accounts=[user.address, self.market2.address],
            foreign_apps=[self.market2.app_id, self.market2.manager_app_id],
            foreign_assets=[self.market2.underlying_asset_id, self.market2.b_asset_id]
