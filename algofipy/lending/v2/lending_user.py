...
class LendingUser:
    ...
    def get_preamble_txns(self, params, target_market_app_id, sender_address=''):
        ...
        for page in range(page_count):
            app_args = [
                bytes(MANAGER_STRINGS.calculate_user_position, "utf-8"),
                int_to_bytes(page),
                int_to_bytes(target_market_app_id),
            ]
            accounts = [self.storage_address]
            foreign_apps = self.opted_in_markets[page * 3 : (page + 1) * 3] + [
                self.lending_client.markets[market_app_id].oracle.app_id
                for market_app_id in self.opted_in_markets[page * 3 : (page + 1) * 3]
            ]
            txns.append(
                ApplicationNoOpTxn(
                    sender_address,
